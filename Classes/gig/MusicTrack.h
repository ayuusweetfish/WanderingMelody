...
class MusicTrack {
public:
    MusicTrack() : _soundbank(nullptr), _chainedNote(-1) { }
    ...
    flags_t triggerNote(int32_t time, int32_t actualTime, char tag);
...
protected:
    ...
    int _chainedNote;
    int32_t _chainedNoteTime;

    void updateChainedNote(int32_t time, int index);
};
